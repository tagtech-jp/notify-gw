...
wc -c < .run-key.txt                  # 長さを確認（RUN_KEY は 48 バイト）
tail -c 1 .run-key.txt | od -An -tx1  # 末尾1バイトのみ確認。0a / 0d が出たら作り直す
```

**鍵ファイルに対して `xxd` / `hexdump` / `cat` / `od`（全体表示）/ `type`（PowerShell）/
`Get-Content`（PowerShell）を使うことを禁止する（2026-09-25 制定）。**
`xxd file | tail -1` は「末尾バイトだけ見ているつもり」でも、`xxd` は行ごとに
16進とASCII表示の両方を出すため、**その1行に鍵の中身がほぼ丸ごと平文で写り、
画面（＝会話ログ）に残る**。末尾バイトだけを見たいときは `tail -c 1 file | od -An -tx1`
を使う（`tail -c 1` でファイルを1バイトに切り詰めてから `od` に渡すので、鍵の他の
バイトはそもそもパイプに乗らない）。長さは `wc -c` で確認し、中身は決して覗かない。

実害（2026-09-25）: 本節の旧版にあった `xxd .run-key.txt | tail -1` という手順例を
そのまま実行し、notify-gw の RUN_KEY が会話ログに平文で残った。外部への送信・公開はして
いないが、値を画面に出さない原則に反する。指示者（社長）が事後に妥当性を確認し、
「発見して報告し、作業を止めた」対応自体は正しいと判定。ローテーションは実施するが、
複数の送信元を同時に張り替える移行作業（09-29 予定）と合わせて1回で行う方針とした
（個別ローテーションで送信元の一部だけ差し替えると、その1か所が黙って403になり続ける
リスクがあるため。§7「解消されない異常は解消まで毎日報告する」と同じ「気づけない失敗」）。
